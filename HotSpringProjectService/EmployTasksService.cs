...
namespace HotSpringProjectService
{
    public class EmployTasksService : IEmployTasksService
    {
        ...
        public ResMessage UpdateTask(int id, string data,int equId)
        {
            _dbUp.execBySql($"update Equ_Upkeep_Task set upkeep_feedback_info='{data}' , status = 1 where equ_plan_id={id} and equ_id={equId}");
